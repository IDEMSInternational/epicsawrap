...
#' @param override Deprecated. A logical argument, always `FALSE`. Used to indicate whether to calculate the summaries even if stored in the bucket.
#'
...
annual_rainfall_summaries <- function(country, station_id, call = c("climsoft", "googlebuckets"),
  ...
  if (nrow(summary_data) > 0) { # & if override == FALSE
    ...
    if (nrow(file_name) == 0) {
      ...
      #    definitions <- definitions(country = country, definitions_id = definitions_id, summaries = "annual_summaries", file = file)
      # }
    }
...
crop_success_probabilities <- function(country,
  ...
  if ((!is.null(planting_dates) | !is.null(water_requirements) | !is.null(planting_length) | !is.null(start_before_season)) & override == FALSE){
    ...
    #override <- TRUE
    warning("Access to raw data is required")
...
season_start_probabilities <- function(country,
  ...
  if (!is.null(start_dates) & nrow(summary_data) > 0) warning("Cannot calculate start dates")
  
  if (nrow(summary_data) > 0 & is.null(start_dates)) { #& override == FALSE
    file_name <- get_objects_in_bucket(country, definitions_id, timestamp = timestamp)
    if (nrow(file_name) == 0) {
      list_return[[1]] <- (definitions(country, station_id = station_id, summaries = summaries))
    } else {
...
definitions <- function(country, station_id = NULL, definitions_id = NULL, summaries, file = NULL) {
  # fetch the full definition structure
  dd <- get_definitions_data(country = country, station_id = station_id, file = file)

  # helper: get by path "a/b/c" OR by bare name with fallback into annual_summaries
  get_item <- function(dd, key) {
    if (grepl("/", key, fixed = TRUE)) {
      parts <- strsplit(key, "/", fixed = TRUE)[[1]]
      out <- Reduce(function(x, nm) if (is.list(x) && !is.null(x[[nm]])) x[[nm]] else NULL, parts, init = dd)
      return(out)
    } else {
      # try top-level
      if (!is.null(dd[[key]])) return(dd[[key]])
      # fallback: annual_summaries
      # (note the new ones are further nested under "annual_summaries"; same for temperature summaries)
      if (!is.null(dd$annual_summaries) && is.list(dd$annual_summaries) && !is.null(dd$annual_summaries[[key]])) {
        return(dd$annual_summaries[[key]])
      }
      if (!is.null(dd$monthly_temperature_summaries) && is.list(dd$monthly_temperature_summaries) && !is.null(dd$monthly_temperature_summaries[[key]])) {
        return(dd$monthly_temperature_summaries[[key]])
      }
      if (!is.null(dd$annual_temperature_summaries) && is.list(dd$annual_temperature_summaries) && !is.null(dd$annual_temperature_summaries[[key]])) {
        return(dd$annual_temperature_summaries[[key]])
      }
      return(NULL)
    }
  }
  ...
  # collect results
  res <- lapply(summaries, function(k) get_item(dd, k))

  # set names: use last path segment; disambiguate duplicates by using full key
  base_names <- vapply(strsplit(summaries, "/", fixed = TRUE), function(x) tail(x, 1), character(1))
  # disambiguate duplicates if needed
  if (any(duplicated(base_names))) {
    # where duplicated, keep full path name
    dup <- base_names[duplicated(base_names) | duplicated(base_names, fromLast = TRUE)]
    final_names <- base_names
    final_names[base_names %in% dup] <- summaries[base_names %in% dup]
  } else {
    final_names <- base_names
  }
  ...
  # warn about missing
  missing_idx <- vapply(res, is.null, logical(1))
  if (any(missing_idx)) {
    missing_keys <- summaries[missing_idx]
    warning(paste0(
